...
public class DeliveryProducer {
    ...
    public DeliveryProducer(
            ...
            @Value("${webhook.delivery.queue:webhook-deliveries}") String queueName
    ) {
    ...
    public void publish(Delivery delivery, NotificationRequest request) {
        ...
        log.info("producer event={} subscription={} deliveryId={} queue={} callback={} published",
                request.eventId(), delivery.subscription().id(), delivery.id(), queueName, delivery.subscription().callbackUrl());
    }
}
